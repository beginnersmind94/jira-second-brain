...
- `approved` — a **Product Owner** (acting as SME) reviewed the grounded draft and signed off. **This is the gate INTO the Library** — only approved content appears there and feeds quiz/flashcard generation. Approval re-validates grounding live before it is allowed.

**The reviewer is a Product Owner, not the implementation team.** The PM team's product owners are the SMEs for each module. The implementation team (Jaime's team) is *downstream* of this gate — they consume approved guides and assign them to districts. Do not surface approval queues or "awaiting review" actions to implementation users; those belong to POs.

**The deterministic grounding gate is the machine floor** (a draft can't be approved until every citation is verbatim and correctly-tiered). **PO approval is the floor into the Library.** Do not flip to `approved` without an explicit PO decision — the agent never approves autonomously. Unapproved drafts are never shown in the Library; they sit in its "Awaiting review" queue.
...
3. Cross-link: each story links back to the epic; the epic lists all stories

### When changing existing feature code
1. Open the relevant story file(s) in `docs/features/`
...
```
learning-agent/docs/features/
  <feature-slug>/
    EPIC.md          ← problem, solution, design decisions, story index
    STORY-001-*.md   ← one file per acceptance-criterion cluster
    STORY-002-*.md
    …
```
...
- **SCORM 1.2 export + xAPI emission are scaffolded (V2).** `GET /api/tracks/{id}/scorm` returns a SCORM 1.2 zip; xAPI `completed`/`progressed` statements emitted on cert + module-done; stub mode logs to `logs/xapi-stub.jsonl`; production mode POSTs to `LRS_ENDPOINT` with `LRS_KEY` auth. "Export as SCORM" button in track builder. Non-fatal: LRS outage never blocks cert issuance.
- **Template editor + regeneration workflow not built.** V1.5.
- **PDF transcript upload is built** (`pdf_to_md.py` + `app.upload_transcript`): a `.pdf` upload is converted to markdown server-side — reuses `../scripts/guide_text_cleanup` (header/footer strip, Symbol-font bullet glyph, de-hyphenation, column reflow, **multi-column warning**), adds scanned/encrypted-PDF detection and an 80-page / 25s cap, then stores a normal `.md` transcript. **Word (`.docx`) upload** is still V1.5.
- **Existing guides imported into the Library.** `import_guides.py` converts the 86 curated SchoolCafé/Family Hub guides (`raw/guides/markdown/**` via `manifest.csv`) into approved Library resources tagged `origin: internal` — honest provenance (human-authored, **not** gate-grounded). A **Source** filter in the Library separates internal (Cybersoft) from outside-vendor (ICN) content; ICN stays link-out + credited (never reproduced).
...
- **SSO scaffold implemented** (`auth.py`): `CurrentUser` dataclass + `get_current_user()` FastAPI `Depends()` are the single identity interface. Demo bypass via `X-Demo-User` header (john-cashier / dana-director / sam-trainer). Production JWT integration with SchoolCafé/PrimeroEdge is pending (ADR-001 §Auth). All identity reads go through `get_current_user()` — hardcoding district or user identity in routes is prohibited.
