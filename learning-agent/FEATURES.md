...
| ICN video embedding (C1) | License-aware ICN embed: youtube-nocookie iframe for `embed_only`/`download_allowed`; link-out card for `link_only`. Attribution band structural (non-removable). Offline Wi-Fi fallback (thumbnail+link after 6s or `OFFLINE_DEMO=1`). `cbAddVideo()` replaced by ICN picker — no raw URL entry, lesson stores ICN asset ID (`ref`), never a URL. `EXTERNAL_LEARNING=0` disables gracefully. | **Real — license-aware ICN embed (youtube-nocookie) or link-out card per `license_posture`; attribution band non-removable; offline fallback chrome** | `demo_app.py` `/api/icn`, `/api/icn/<id>`, `/api/config` (`offline_demo`); `index.html` `renderVideoCard`/`loadAndRenderVideoCard`/`cbAddVideo`/`cbPickVideo`; `data/icn/data/`; `tests/test_video_embedding.py` |
