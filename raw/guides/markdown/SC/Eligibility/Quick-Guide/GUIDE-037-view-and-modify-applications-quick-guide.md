...
curated_against_raw_sha: "9989433df403a250"
...
> Seeded from the raw extraction. Edit freely. When the raw drifts, review the diff and update this file, then mark it reviewed with `python scripts/mark_guide_reviewed.py GUIDE-037`.
...
3. Optionally select the Show Only Active Benefit Applications or Show Only Error-Prone Applications checkboxes to limit the grid
   - Active benefit applications are those where at least one student is still maintaining eligibility from the application
   - Both checkboxes are unselected by default
4. Click the APPLY button
   Results display below.
   Applicant Address, Applicant Email, Home Phone, and Work Phone columns are available for the grid. They are hidden by default; enable them through the grid's custom views.
...
## Action(s) for Pending Applications: View, Deny, Refuse Benefits, Delete, History
...
2. Navigate to the Summary section to view the Pending Reasons
   Deny
1. Click the Deny icon for the desired pending application to close it out as Denied
   The application is marked as Denied for incomplete information, students remain at Paid (subject to adverse action rules), and the application moves off the Pending Applications tab.
   A success message displays: "Application denied successfully."
   A denial notice can be printed for the application.
   Refuse Benefits
1. Click the Refuse Benefits icon for the desired pending application
   The application is marked as Denied because the applicant refused benefits, student eligibility records are updated to Reduced/Paid – Refused Benefits (subject to adverse action rules), and the application moves off the Pending Applications tab.
   A success message displays: "Application benefits were refused."
   A denial notice can be printed for the application.
   Delete
...
## Action(s) for Pending Students: Search, Match, Edit, View

When more than one student is available to link against the same Pending student, a MATCH button is shown for each candidate; select one of the candidates to link the student. The application's details are only updated after PROCESS MATCHES is clicked. The Match Details screen also shows the candidate's Status and Eligibility.
