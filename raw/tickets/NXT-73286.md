...
**AC5 — Category pill.** Four options: All (default), Imports, Exports, Reports. Exports is present but shows no rows until configured exports are built.
...
**AC15 — GL Accounts confirmation dialog.** Dialog shows import name, account/category counts, full list of everything that will be deleted. Warning per sec 4.3. Mandatory comment (min 10 chars). Buttons: Cancel (default) / Reverse import (destructive).
